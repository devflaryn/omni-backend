...
export default function omniExec(req, res, next) {
  ...
  // `http://179.198.197.7/` so a source `.../api/x` arrives as `//api/x`).
  ...
  if (p === '/neo_versions_required' || p.endsWith('/neo_versions_required')) {
    ...
    for (const rv of ['2.731.944', '2.732.1043', '2.726.1142', '2.733.988', '2.734.917', '2.735.1138']) lines.push(`${rv}|${lowApk}`);
